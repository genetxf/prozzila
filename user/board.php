                                ...
                                ?>
                                    ...
                                    <div id="view_card_modal<?= $cardid ?>" class="modal custom-modal fade" style="display: none;" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                                            <?php
                                            // Fetch data from the "cards" table
                                            $tasksSql1 = "SELECT * FROM prozzila.assignment WHERE employee_id = '$logid'";
                                            $tasksResult1 = $conn->query($tasksSql1);

                                            // Check if the query was successful
                                            if ($tasksResult1) {
                                            // Loop through the results (assuming there is only one row for an employee)
                                            while ($row = $tasksResult1->fetch_assoc()) {
                                                $employee_id = $row['employee_id'];
                                            }

                                            // Fetch tasks for the current employee from the tasks table
                                            $tasksSql2 = "SELECT * FROM tasks WHERE card_id = $cardid AND id = $employee_id";
